...
@ccclass('VirtualSingle')
export class VirtualSingle extends Component {
    @property(VList)
    private vList: VList;
    ...
    private onVerticalRenderHandler(index: number, item: Node): void {
        item.getComponent(VerticalItem).updateItem(index, this._vDatas[index]);
    }

    private onHorizontalRenderHandler(index: number, item: Node): void {
        item.getComponent(HorizontalItem).updateItem(index, this._hDatas[index]);
    }
