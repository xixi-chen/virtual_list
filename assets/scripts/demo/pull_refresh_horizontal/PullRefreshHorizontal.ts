...
import { PullRefreshHorizontalItem } from './PullRefreshHorizontalItem';
const { ccclass, property } = _decorator;

@ccclass('PullRefreshHorizontal')
export class PullRefreshHorizontal extends Component {
    @property(VList)
    ...
    private onFooterRenderHandler(index: number, item: Node): void {
        item.getComponent(PullRefreshHorizontalItem).updateItem(index, this._footerDatas[index]);
    }

    private onHeaderRenderHandler(index: number, item: Node): void {
        item.getComponent(PullRefreshHorizontalItem).updateItem(index, this._headerDatas[index]);
    }
    ...
    private onFooterPullRefreshHandle(pullEventType: string): void {
        switch (pullEventType) {
            ...
            case PullEventType.PULLING:
                ...
                this.txtFooterTips.string = "左拉加载";
                break;

            case PullEventType.READY:
                this.txtFooterTips.string = "松开加载";
                break;

            case PullEventType.LOADING:
                this.txtFooterTips.string = "加载中";
                this.scheduleOnce(() => {
    ...
    private onHeaderPullRefreshHandle(pullEventType: string): void {
        switch (pullEventType) {
            ...
            case PullEventType.PULLING:
                ...
                this.txtHeaderTips.string = "右拉加载";
                break;

            case PullEventType.READY:
                this.txtHeaderTips.string = "松开加载";
                break;

            case PullEventType.LOADING:
                this.txtHeaderTips.string = "加载中";
                this.scheduleOnce(() => {
